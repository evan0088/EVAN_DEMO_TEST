...
entity_data = pd.read_csv(os.path.join(bast_path, 'data/entity_list.csv'), encoding='utf-8')
print(f'entity_data--》{entity_data.head()}')

# TODO：对每句句子识别并匹配实体
# 读取包含待处理句子的文件（valid_data.csv）
valid_data = pd.read_csv(os.path.join(bast_path, 'data/valid_data.csv'), encoding='utf-8')
...
split = s.split('|')
print(split)
print(collections.Counter(split).items())

# TODO： 同一个实体出现多次的时候，才需要进行消歧
for k, v in collections.Counter(split).items():
    if v > 1:
...
def get_entityid(query):
    id_start = 1001  # 假设实体ID从1001开始
    # TODO 构造query
    a_list = [' '.join(jieba.cut(query))]  # 对输入句子分词
    print(f'a_list--》{a_list}')
    # TODO 把query转化成向量
    query_vector = vectorizer.transform(a_list)
    print(query_vector)  # 将句子转换为TF-IDF特征
    # TODO 计算query和doc的相似度
    similarity = cosine_similarity(query_vector, X)
    print(similarity)  # 计算句子与所有实体描述的余弦相似度
    res = similarity[0]  # 获取相似度结果
    # TODO 排序，取最大值的所在的索引
...
# TODO ：循环处理每个句子， 对每个句子调用get_entityid, 获取每个句子中的实体
...
for sentence in valid_data['sentence']:
    ...
    for keyword in keyword_list:
        if keyword in sentence:  # 如果句子中包含关键词
            ...
            print(f'k_len--》{k_len}')
            # TODO 记录关键词的字符串:  0-5:1001|8-10:1001
            ss = ''
