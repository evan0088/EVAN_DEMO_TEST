...
#### 🎯 直观理解：学习率 & 权重衰减（Learning Rate & Weight Decay）

> 这是深度学习中最需要**直觉**的两个超参，用人话一次性讲透：

```
🧗 学习率（Learning Rate） = 你每次迈出的"步子大小"

   如果步子太大（LR 太大），你可能直接跨过了山谷，走到对面的山坡上，来回震荡。
   如果步子太小（LR 太小），你走一天也走不出几米，天黑前（训练结束）根本走不到谷底。

   ┌─────────────────────────────────────────────────────────┐
   │  LR 太大 ──→ 跨越最优值，loss 上蹿下跳，甚至 NaN       │
   │  LR 适中 ──→ 不紧不慢，稳步走向最低点 🌟               │
   │  LR 太小 ──→ 慢如乌龟，可能永远到不了                   │
   └─────────────────────────────────────────────────────────┘
```
...
```
🏋️ 权重衰减（Weight Decay） = 你身上背的"沙袋"或地面的"粘滞阻力"

   它不直接告诉你往哪个方向走，但它会限制你的动作。
   如果没有阻力（WD=0），你可能会因为惯性（过拟合）在某个小坑里疯狂打转，
   或者因为走得太猛冲出悬崖。
   加上适当的阻力（WD>0），你的动作会变得平滑、克制，走得更稳，
   从而更大概率找到真正的大谷底（泛化到测试集）。

   ┌─────────────────────────────────────────────────────────┐
   │  WD=0     ──→ 自由奔跑，容易在训练数据的小坑里过拟合     │
   │  WD 适中  ──→ 有阻力但不阻碍前进，泛化能力最好 🌟       │
   │  WD 太大  ──→ 寸步难行，模型学不到任何东西（欠拟合）     │
   └─────────────────────────────────────────────────────────┘
```

| 对比维度 | 学习率 LR | 权重衰减 WD |
|---------|----------|------------|
| **作用对象** | 梯度方向（往哪走） | 权重大小（走多远被拉回来） |
| **核心公式** | `w = w - lr × ∇L` | `w = w - lr × ∇L - lr × wd × w` |
| **调大效果** | 学得快，但可能震荡 / 爆炸 | 模型简单，但可能欠拟合 |
| **调小效果** | 收敛慢，可能停滞 | 约束弱，可能过拟合 |
| **典型值** | Adam: 1e-4 ~ 1e-2（默认 1e-3） | 1e-4 ~ 1e-2（Transformer 常用 0.01） |

> 🔑 **一句话记忆口诀**：**LR 管"学多快"，WD 管"别太浪"**。LR 太大模型疯，WD 太小模型飘。
...
7. **Q：权重衰减（Weight Decay）和 L2 正则化是什么关系？**
   A：本质相同——都是约束权重大小防止过拟合。L2 正则化在损失函数中加 `λ||w||²` 项；Weight Decay 直接在更新时减去 `lr × wd × w`。对于 SGD 二者等价，对于 Adam 则不完全相同（AdamW 使用了**解耦权重衰减**，效果更好）。

8. **Q：Weight Decay 什么时候用？设多大？**
   A：过拟合时加（训练 loss 远低于验证 loss）；欠拟合时去掉。典型值 1e-4 ~ 1e-2。BERT/Transformer 标配 `AdamW + lr=1e-4/2e-5 + weight_decay=0.01`。**LR 和 WD 通常联动调整**：LR 大时 WD 也可适当加大，LR 小时 WD 也要收敛。
