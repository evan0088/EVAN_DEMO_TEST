...
```
# Bahdanau 注意力公式
attn_scores = v^T · tanh(W₁·Q + W₂·K + b)
attn_weights = softmax(attn_scores)
context = attn_weights · V
```
...
```python
# 拼接 Q 和 K
q_k_cat = torch.cat([query, key], dim=-1)
# 通过线性层计算相似度分数
attn_scores = self.attn(q_k_cat)  # Linear层
# Softmax 归一化
attn_weights = torch.softmax(attn_scores, dim=-1)
# 加权求和得到上下文向量
attn_c = torch.bmm(attn_weights, value)
```
...
| 特性 | 加性注意力 | 乘性注意力（点积） |
|------|----------|------------------|
| 计算方式 | `Linear(cat(Q, K))` | `Q · K^T` |
| 速度 | 较慢 | **更快**（矩阵乘法优化好） |
| 参数 | 需要额外参数 | 无需额外参数 |
| 维度要求 | Q、K维度可不同 | Q、K维度必须相同 |
| 数值稳定性 | 更稳定 | 高维时需缩放（Scaled） |
